...
class OverlayController(private val context: Context, private val onScan: () -> Unit) {
    ...
    private lateinit var callerMeta: TextView
    private lateinit var body: TextView
    private lateinit var status: TextView
    private var positionX = 18
    private var positionY = 72
    ...
    fun show(session: CallSession) {
        ...
        try {
            ...
            root = LinearLayout(context).apply {
                ...
                background = card(0xf20b111d.toInt(), 20, 0x33ffffff)
            ...
            Log.e("OverlayController", "Error showing overlay", e)
        }
    }
    ...
    fun update(verdict: Verdict) {
        ...
        try {
            ...
            if (verdict.state == GuardState.HIGH_RISK) {
                try {
                    context.getSystemService(Vibrator::class.java)?.vibrate(VibrationEffect.createWaveform(longArrayOf(0, 160, 90, 240), -1))
                } catch (e: Throwable) {
                    Log.w("OverlayController", "Vibration failed", e)
                }
            }
        } catch (e: Throwable) {
            Log.e("OverlayController", "Error updating overlay", e)
        }
    }

    fun hide() {
        try {
            root?.let { windows.removeView(it); root = null }
        } catch (e: Throwable) {
            Log.e("OverlayController", "Error hiding overlay", e)
        }
    }

    private fun apply(view: LinearLayout, color: Int, headline: String, message: String) {
        view.background = card(color, 20, 0x55ffffff)
        title.text = headline
        title.setTextColor(Color.WHITE)
        body.text = message
    }

    private fun card(color: Int, radius: Int, border: Int) = GradientDrawable().apply {
        setColor(color)
    ...
    private fun drag() = object : View.OnTouchListener {
        var downX = 0f; var downY = 0f; var startX = 0; var startY = 0
        override fun onTouch(v: View, event: MotionEvent): Boolean = when (event.action) {
            MotionEvent.ACTION_DOWN -> { downX = event.rawX; downY = event.rawY; startX = positionX; startY = positionY; true }
            MotionEvent.ACTION_MOVE -> { positionX = startX + (downX - event.rawX).toInt(); positionY = startY + (event.rawY - downY).toInt(); root?.let { windows.updateViewLayout(it, params()) }; true }
            else -> true
        }
    }
}
